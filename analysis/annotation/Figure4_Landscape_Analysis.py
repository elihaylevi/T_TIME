...
# The notebook-derived reference file spells this bucket "Human (Homo sapiens)"; the
# scripted 13 -> 14 chain inherits VDJdb/TRAIT spellings ("HomoSapiens", no space).
# Both must be dropped, otherwise the unannotated human self bucket survives into the
# top-9 species and category_map raises a KeyError.
HUMAN_SPECIES = {"Human (Homo sapiens)", "HomoSapiens", "Homo Sapiens", "Homo sapiens"}
df = df[~df['Epitope_species_norm'].isin(HUMAN_SPECIES)].copy()
...
category_map = {
    ...
    # Belt-and-braces: HUMAN_SPECIES above removes these rows, so this entry is only
    # reached if a merged file carries a human spelling not listed there.
    'HomoSapiens': 'Self/Cancer',
    # The nine keys above cover the top-9 of the notebook-derived reference file. The
    # scripted 13 -> 14 chain yields a slightly different top-9 (it lacks the notebook's
    # richer Melanoma/Merkel/T1D grouping), so YFV and HCV can enter it. Both are viral.
    'YFV': 'Viral', 'HCV': 'Viral'
}
